(function(angular) {
    ...
    BypassController.$inject = ['dataservice', '$window', '$scope', 'userdataservice','$location'];

    function BypassController(dataservice, $window, $scope, userdataservice, $location) {
        ...
        */
        ...
        $scope.$watch('vm.user', function(current, original){
            ...
            if(current.captcha == null || current.captcha == ""){
                ...
                if(current.email != "" || (typeof current.email != "undefined")){
                    vm.shutDown = false;
                    vm.selectedIndex = 1;
                    oldUser(vm.user);
                    generateNumbers();
